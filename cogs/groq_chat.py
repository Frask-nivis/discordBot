...
SEE_HISTORY_DEFAULT_MINUTES = 180
SEE_HISTORY_MAX_MINUTES = 360
...
class ToolActivity:
    ...
    _frames = (
        ("â—Œ", "Menyiapkan permintaan"),
        ("â—", "AI sedang berpikir"),
        ("â—Ž", "Menjalankan tool"),
        ("â—‰", "Menerima hasil"),
    )
...
class GroqChat(commands.Cog):
    ...
    async def _see_history(
        self,
        channel: object | None,
        minutes: int = SEE_HISTORY_DEFAULT_MINUTES,
        query: str = "",
        fallback_context: str | None = None,
    ) -> str:
        """Return bounded, speaker-labelled history for the active channel."""
        if channel is None or not hasattr(channel, "history"):
            return fallback_context or "Channel aktif tidak tersedia untuk membaca history."

        try:
            minutes = max(1, min(int(minutes), SEE_HISTORY_MAX_MINUTES))
        except (TypeError, ValueError):
            minutes = SEE_HISTORY_DEFAULT_MINUTES
        ...
        try:
            ...
            return fallback_context or "History channel tidak dapat dibaca karena permission atau error Discord."

        if not entries:
            return "Tidak ada pesan yang cocok di history channel pada rentang waktu tersebut."

        return f"HISTORY CHANNEL ({minutes} menit terakhir)\n" + "\n\n".join(entries)[-MAX_CHANNEL_CONTEXT_CHARS:]
    ...
    def _tool_definitions(self) -> list[dict]:
        return [
            ...
            },
            {
                ...
                "function": {
                    "name": "seehistory",
                    "description": "Baca history channel Discord aktif saat konteks belum cukup. Jika user tidak menyebut rentang, gunakan sekitar 3 jam; maksimal 6 jam dan 100 pesan.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "minutes": {"type": "integer", "minimum": 1, "maximum": 360, "description": "Rentang history dalam menit; gunakan 180 jika user tidak menyebut durasi."},
                            "query": {"type": "string", "description": "Filter kata/frasa opsional."},
                        },
                    },
                },
            },
    ...
    async def _tool_executor(
        ...
        if name == "seehistory":
            return await self._see_history(
                channel,
                arguments.get("minutes", SEE_HISTORY_DEFAULT_MINUTES),
                ...
                for call in tool_calls:
                    try:
                        ...
                        if activity is not None:
                            ...
                            elif call.function.name == "seehistory":
                                detail = f"Membaca history {arguments.get('minutes', SEE_HISTORY_DEFAULT_MINUTES)} menit..."
                        ...
                        if inspect.iscoroutinefunction(self._tool_executor):
                            if len(parameters) >= 6:
                                result = await self._tool_executor(
                                    call.function.name, arguments, guild, requester, channel, channel_context
                                )
                                ...
                                result = await self._tool_executor(call.function.name, arguments)
